# Modi Harness Documentation

## Project Governance

- [Product positioning](./project/positioning.md)
- [Versioning and naming policy](./project/versioning.md)

## Architecture

Current maintainer documentation:

- [Architecture overview](./architecture/README.md)
- [Agent and Skill](./architecture/agent-and-skill.md)
- [Execution runtime](./architecture/execution-runtime.md)
- [Context and Memory](./architecture/context-and-memory.md)
- [Tools and Policy](./architecture/tools-and-policy.md)
- [Workspace and Trace](./architecture/workspace-and-trace.md)
- [Output and Hooks](./architecture/output-and-hooks.md)

## Guides

- [CLI](./guides/cli.md)
- [Agents and task protocol](./guides/agents.md)
- [Builtin Tools](./guides/builtin-tools.md)
- [Plugins](./guides/plugins.md)

## Reference

- [Types](./reference/types.md)
- [Claude Code system prompt (original)](./reference/claude-code-system-prompt-original.md)

Detailed design history and implementation plans live under
[`superpowers/`](./superpowers/). Release changes live in the repository root
[`CHANGELOG.md`](../CHANGELOG.md).
