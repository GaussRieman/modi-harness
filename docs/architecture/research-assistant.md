...
| 每个 provider 返回的候选数 | 最多 6 |
| 候选页面抓取尝试 | 最多 12，抓取并发最多 5 |
| 最终可用来源 | 最多 6 |
| `public_web_search` 每个 task 的有效调用次数 | 最多 4；失败调用不占有效额度 |
...
候选先在每个实体池内独立排序，同一实体优先保留不同域名，再轮询分配最多 12 个抓取槽位；两个非空实体池都会在任一实体获得第二个槽位前先获得一次尝试。URL 去重遇到共享候选时会继续寻找该实体的下一个候选，不会让另一个实体失去覆盖。最多 5 个页面并发抓取，前序候选失败不会阻止后续候选进入结果。
...
Research Assistant 默认使用 Bing RSS、百度 HTML 和 DuckDuckGo HTML/Lite
等免费公开网页入口。若配置 `MODI_DOUBAO_SEARCH_API_KEY`，搜索 fan-out 会额外启用
火山方舟 Web Search 作为候选发现 provider；未配置或调用失败时，免费 provider 不受影响。
豆包返回的回答文本不进入证据链，只有 URL 引用会和其他 provider 一样经过候选排序、网页抓取
及 `verify_claim_evidence`。每个豆包请求固定限制关键词和工具调用次数，避免付费搜索扩散。
详见 `docs/reference/doubao_web_search.md` 和 `docs/superpowers/specs/2026-07-20-doubao-web-search-provider-design.md`。

## 12. 状态、Checkpoint 与 Trace
