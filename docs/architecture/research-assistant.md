...
flowchart TD
    ...
    G --> C[autonomous: synthesize_report]
...
### 7.5 Synthesis 与确定性收口

Goal 验证通过后，`synthesize_report` 只收到 accepted `committed_results`。模型只生成：

```yaml
direct_answer: 跨维度综合结论
limitations:
  - 总体限制
```

它不能提交 `key_findings`、citations、evidence 或 provenance。`finalize_report` 随后调用 `build_evidence_graph(report, committed_results)`，确定性提取 canonical Findings、合并真实 citations 和 limitations，并生成 Mermaid evidence graph。模型伪造或回忆的 URL 不会进入最终输出。
...
- provenance（每次 search 的 `search_id`、structured searches、usable URLs 和 fresh-time context）；
- 限制条件。
...
Deep Research 的模型在 `synthesize_report` 节点最终只提交：
...
Harness 从父 Runtime 已接受的 `committed_results` 自动加入：
...
这避免模型在收尾时重新抄写 Finding，导致结论、引用、provenance 或置信度漂移。CLI 再将引用编号放到对应 evidence claim 旁边，而不是只在末尾输出无关联 URL 列表。

### 10.1 `finalize_report`：证据图谱

`synthesize_report` 完成后，`deep_research` 还有一个确定性 `operation` Node —— `finalize_report`，执行 `build_evidence_graph` Operation：
...
它是纯函数：只从 `committed_results` 提取已接受 Finding，为每个问题和每条不重复的证据来源生成 Mermaid 节点；`supporting` 关系画实线、`contradicting` 画虚线，`sourced`/`limited` 问题使用不同样式。输出附加 `evidence_graph` 字段（`flowchart LR` 文本）。因为它忽略综合模型自报的 Findings 和 citations，最终证据始终回溯到父 Task Verifier 已接受的 submission。
...
- 综合模型伪造的 Finding、citation 或 evidence 不进入最终结果；
...
| `agents/research_assistant/workflows/deep_research.yaml` | Intent、Task Graph、综合与确定性收口 |
