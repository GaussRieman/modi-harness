...
_USER_AGENT = "ModiHarness-ResearchAssistant/0.8"
...
def _query_variants(subject: str, question: str) -> list[str]:
    core = _subject_aliases(subject)[1]
    dimension = _question_dimension(question)
    variants = [subject, f"{core} {dimension or '公司 官网 产品 技术'}"]
    out: list[str] = []
...
def _search_provider(provider: str, query: str) -> dict[str, Any]:
    ...
    try:
        if provider == "bing_rss":
            ...
            results = _search_html(provider, search_url)
...
def _search_html(provider: str, search_url: str) -> list[dict[str, str]]:
    payload, _final_url, content_type = _read_url(search_url, timeout=6, limit=1_000_000)
    parser = _AnchorExtractor()
    parser.feed(_decode(payload, content_type))
    results: list[dict[str, str]] = []
    seen: set[str] = set()
    for anchor in parser.anchors:
        if provider == "duckduckgo" and "result__a" not in anchor["class"]:
            continue
...
def _read_url(url: str, *, timeout: int, limit: int) -> tuple[bytes, str, str]:
